...
class Deskew:
    ...
    def __init__(self, input_file, display_image, output_file=None, r_angle=0):
    ...
    def deskew(self):
        ...
        resize = (abs(rot_angle) > 45 and abs(rot_angle) < 135)

        self.deskewed_image = rotate(img, rot_angle, resize=resize)
        # self.deskewed_image = rotate(img, rot_angle, resize=False)

        if self.display_image:
            self.display(self.deskewed_image)

        self.deskewed_image = self.deskewed_image*255

        if self.output_file:
            self.saveImage(self.deskewed_image*255)
